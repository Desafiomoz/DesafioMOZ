...
export async function processarCobrancaPaga(chargeId) {
  if (await jaProcessado(chargeId)) return { credited: true, diag: "já estava processada" };
  ...
  if (!pedido || !pedido.item || !pedido.email) {
    ...
    return { credited: false, diag: "pedido não encontrado em pedidosLoja/" + chargeId };
  }

  await creditarCompra(pedido.item, pedido.email);
  await marcarProcessado(chargeId, pedido.item, pedido.email);
  return { credited: true, item: pedido.item, email: pedido.email, diag: "creditado com sucesso" };
}
